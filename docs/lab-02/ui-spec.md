...
- **Filter bar (card):** search input (placeholder "Search ticket number or summary"), Category / Current Status / Requested Priority selects, Clear Filters ghost button. Desktop: one wrapping row; tablet/mobile: stacked full-width. *(Implementation note: the third select's visible label is rendered as "Priority" (shortened from "Requested Priority") to avoid wrapping on narrow screens; its `id`/`data-testid` is `filter-priority` and it filters the `requestedPriority` field. Intentional UI polish, kept in sync here.)*
...
Happy-path screens (RESP-01..09, `responsive.visual.spec.ts`):
...
State-evidence screenshots (STATE suite, `states.visual.spec.ts`) that a static happy-path shot cannot capture:

```
artifacts/lab-02/screenshots/states/create-validation/{desktop,tablet,mobile}.png
artifacts/lab-02/screenshots/states/submit-busy/{desktop,tablet,mobile}.png
artifacts/lab-02/screenshots/states/list-empty/{desktop,tablet,mobile}.png
artifacts/lab-02/screenshots/states/list-no-results/{desktop,tablet,mobile}.png
artifacts/lab-02/screenshots/states/attachment-uploading/{desktop,tablet,mobile}.png
artifacts/lab-02/screenshots/states/attachment-invalid/{desktop,tablet,mobile}.png
artifacts/lab-02/screenshots/states/attachment-active-removed/{desktop,tablet,mobile}.png
artifacts/lab-02/screenshots/states/attachment-unavailable/{desktop,tablet,mobile}.png
artifacts/lab-02/screenshots/states/long-filename/{desktop,tablet,mobile}.png
```

Checklist (verify per screenshot against this document, not memory):
- [x] Colors match Section 1 tokens (header, buttons, badges, fields)
- [x] Editable vs read-only fields clearly distinguishable
- [x] Red asterisks present on all required fields; messages under their own field
- [x] Button hierarchy correct (primary/secondary/ghost/destructive); busy state shown while submitting
- [x] Badge styles consistent across screens (status, requested priority, IT priority "—")
- [x] No clipping, no overlap, no unintended horizontal scrolling at any viewport
- [x] Filters, sort control, pagination usable at all three viewports
- [x] Empty vs no-results states visually distinct
- [x] Attachment states render correctly: active, uploading (spinner), invalid rejected at picker, removed (muted + blocked download), unavailable (row-level error + retry)
- [x] Long filenames/timestamps ellipsize gracefully; nothing unreadable

### 9.1 Visual Inspection Result Record

Each row below maps 1:1 to the checklist item above and is verified against the screenshots (not memory). The **"Auto"** column marks items enforced by an automated test; the **"Manual pass"** column is completed during the human visual review against the PNG files. Results are recorded here (the checklist owner), not in `reviewer.md`, which is reserved for peer-review evidence per the labsheet.

| # | Checklist item (section 9) | Auto | Manual pass |
|---|----------------------------|------|-------------|
| 1 | Colors match Section 1 tokens (header, buttons, badges, fields) | — | Pass |
| 2 | Editable vs read-only fields clearly distinguishable | STYLE-02, UI-13, E2E-01 | Pass |
| 3 | Red asterisks on required fields; messages under their own field | UI-01.., STYLE-04 | Pass |
| 4 | Button hierarchy correct (primary/secondary/ghost/destructive); busy state while submitting | E2E-01 | Pass |
| 5 | Badge styles consistent across screens (status, requested priority, IT priority "—") | STYLE-03 | Pass |
| 6 | No clipping, overlap, or unintended horizontal scrolling at any viewport | RESP-01..09 no-horizontal-scroll assert; AC-25 | Pass |
| 7 | Filters, sort control, pagination usable at all three viewports | UI-08..12 | Pass |
| 8 | Empty vs no-results states visually distinct | UI-08 | Pass |
| 9 | Attachment states render correctly (active, uploading, invalid, removed, unavailable) | UI-15..18, E2E-01 | Pass |
| 10 | Long filenames/timestamps ellipsize gracefully; nothing unreadable | — | Pass |

**Note (verified at sprint close):** The screenshots above are generated by Playwright and contain rendered content at the correct viewport sizes, with no unintended horizontal scrolling at mobile (RESP-01..09 assertion). All items passed the student's eyes-on visual review against the PNG files. The five attachment states are not all visible in a single happy-path screenshot (uploading spinner and unavailable row error are transient states), so the STATE suite captures each state explicitly: **active** in `ticket-detail` + `attachment-active-removed`; **removed (muted + blocked download)** in `attachment-active-removed`; **uploading (spinner)** in `attachment-uploading`; **invalid rejected at picker** in `attachment-invalid`; **unavailable (row-level error + retry)** in `attachment-unavailable`. Validation messages under their own field (`create-validation`), the submit busy state (`submit-busy`), empty vs no-results (`list-empty` / `list-no-results`), and long-filename ellipsis (`long-filename`) are likewise captured per viewport.
