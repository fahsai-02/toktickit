...
// Lab 2 attachment behavior migrated to the authenticated contract (Issue 18):
// no `requesterId` fields; `uploadedByRequesterId` is the session-mapped legacy
// Requester row (BR-03, api-spec section 4.4-4.6).
...
describe("POST /api/tickets/:id/attachments", () => {
  ...
  it("API-14: uploaded file appears in ticket detail attachments", async () => {
    ...
    const names = res.body.data.attachments.map(
      (a: { originalFileName: string }) => a.originalFileName
    );
  ...
  it("returns 400 when file is missing", async () => {
    const res = await agent.post(`/api/tickets/${ticketId}/attachments`);
...
describe("GET /api/attachments/:id/download", () => {
  ...
  it("API-15: returns 200 with binary content for active attachment", async () => {
    const res = await agent.get(`/api/attachments/${activeAttachmentId}/download`);
  ...
  it("API-17: returns 410 for removed attachment", async () => {
    await agent
      ...
      .send({ removalReason: "Wrong file attached" });
  ...
  it("returns 404 for unknown attachment id", async () => {
    const res = await agent.get("/api/attachments/99999/download");
...
describe("DELETE /api/attachments/:id", () => {
  ...
  it("API-18: soft-removes an active attachment with valid reason", async () => {
    const res = await agent
      .delete(`/api/attachments/${activeAttachmentId}`)
      .send({ removalReason: "Attached wrong file" });
  ...
  it("API-19: removed attachment metadata persists after soft removal", async () => {
    await agent
      .delete(`/api/attachments/${activeAttachmentId}`)
      .send({ removalReason: "Wrong attachment" });

    const detailRes = await agent.get(`/api/tickets/${ticketId}`);
  ...
  it("API-20: file still exists on disk after soft removal", async () => {
    ...
    const deleteRes = await agent
      ...
      .send({ removalReason: "Test retention" });
  ...
  it("API-21: returns 400 when removing an already removed attachment", async () => {
    await agent
      .delete(`/api/attachments/${activeAttachmentId}`)
      .send({ removalReason: "First removal" });
  ...
  it("returns 403 when removing another requester's attachment", async () => {
    const res = await otherAgent
      .delete(`/api/attachments/${activeAttachmentId}`)
      .send({ removalReason: "Not my attachment" });
  ...
  it("returns 400 when removalReason is missing", async () => {
    const res = await agent.delete(`/api/attachments/${activeAttachmentId}`).send({});
  ...
  it("returns 404 for unknown attachment id", async () => {
    const res = await agent
      ...
      .send({ removalReason: "Unknown attachment" });
